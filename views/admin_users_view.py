...
def show_admin_users(page, user_id, role, name):
    ...
    def format_duration(login_time, logout_time, status):
        if not login_time:
            return "—"

        start = login_time
        end = logout_time if logout_time else datetime.now()
    ...
    def role_badge(role_value):
        role_map = {
            "admin": ("Admin", "#1E3A8A"),
            "faculty": ("Faculty", "#4CAF50"),
            "student": ("Student", "#F59E0B"),
        }
        label, color = role_map.get(role_value, (str(role_value).title(), "#6B7280"))
    ...
    def create_session_row(session):
        return ft.Container(
            content=ft.Row(
                [
                    ft.Container(
                        content=ft.Text(session["full_name"], weight=ft.FontWeight.W_600),
                        width=210
                    ),
                    ft.Container(
                        content=role_badge(session["role"]),
                        width=100
                    ),
                    ft.Container(
                        content=ft.Text(format_dt(session["login_time"])),
                        width=210
                    ),
                    ft.Container(
                        content=ft.Text(format_dt(session["logout_time"])),
                        width=210
                    ),
                    ft.Container(
                        content=ft.Text(
                    ...
                    ft.Container(
                        content=status_badge(session["status"]),
                        width=120
                    ),
                ],
            ...
            bgcolor="#F7FBEF" if session["status"] == "Online" else ft.Colors.WHITE,
            border=ft.border.only(bottom=ft.BorderSide(1, "#E5E7EB"))
        )

    def build_session_history_section():
        try:
            SessionLogModel.mark_stale_sessions_offline(timeout_minutes=5)
        except Exception:
            pass

        try:
            stats = SessionLogModel.get_session_stats()
        except Exception:
            stats = {
                "online_now": 0,
                "total_sessions": 0,
        ...
        try:
            sessions = SessionLogModel.get_session_history(limit=30)
        except Exception:
            sessions = []

        stat_cards = ft.Row(
            [
                create_stat_card("Online Now", stats.get("online_now", 0), ft.Icons.CIRCLE, "#4CAF50"),
                create_stat_card("Logins Today", stats.get("logins_today", 0), ft.Icons.LOGIN, "#3B82F6"),
                create_stat_card("Total Sessions", stats.get("total_sessions", 0), ft.Icons.HISTORY, "#8B5CF6"),
            ],
            spacing=15,
            wrap=True,
        ...
        table_header = ft.Container(
            content=ft.Row(
                [
                    ft.Container(ft.Text("User", weight=ft.FontWeight.BOLD), width=210),
                    ft.Container(ft.Text("Role", weight=ft.FontWeight.BOLD), width=100),
                    ft.Container(ft.Text("Login Time", weight=ft.FontWeight.BOLD), width=210),
                    ft.Container(ft.Text("Logout Time", weight=ft.FontWeight.BOLD), width=210),
                    ft.Container(ft.Text("Duration", weight=ft.FontWeight.BOLD), width=100),
                    ft.Container(ft.Text("Status", weight=ft.FontWeight.BOLD), width=120),
                ],
                spacing=10
            ),
        ...
        table_body = ft.Container(
            content=ft.Column(
                controls=[create_session_row(s) for s in sessions] if sessions else [
                    ft.Container(
                        content=ft.Text(
        ...
        return ft.Container(
            content=ft.Column(
                [
                    ...
                    ft.Text(
                        "Monitor who is online, their login time, logout time, and session duration.",
                        size=13,
    ...
    session_history_section = build_session_history_section()
